...
static void	draw_wall(t_game *g, t_ray *ray, t_wall* wall)
{
	...
	// Use px_per_ray from game struct (now a float!)
 	x = (int)(ray->index * g->px_per_ray);
 	x1 = (int)((ray->index + 1) * g->px_per_ray) - 1;  // ← Same fix!
	...
	while (i < wall->line_h)
	{
		tex_x = (int)wall->tx & (wall->text.height - 1);
		tex_y = (int)wall->ty & (wall->text.height - 1);
		color = *(unsigned int *)(wall->text.buffer 
			+ tex_y * wall->text.line_len 
			+ tex_x * (wall->text.bpp >> 3));
